...
#         while True:
#             # 產生 6 碼英數混合代碼 (排除容易混淆的字元如 I, l, 1, O, 0)
...
#     # 新增：驗證班級代碼並回傳 class_id, school_id
...
#     # 修改：排行榜改用 class_id 查詢，而非 account 字串比對
...
#             JOIN Student  AS s2 ON s1.student_id = s2.student_id
#             CROSS JOIN (SELECT @rownum := 0, @rank := 0, @prev_score := NULL) vars
#             WHERE s2.class_id = %s  -- 這裡改成用 class_id 篩選
#             ORDER BY s1.acc_score DESC, s2.student_name ASC
#         ) t
#         ORDER BY r ASC, acc_score DESC, student_name ASC;
#         """
...
#                 # 組合顯示名稱：座號-姓名 (或是存入資料庫的其他欄位，這裡示範存入 student_name)
#                 # 假設 grade_class_str 是 "3年2班"，我們可以存在 name 裡方便辨識，或是只存姓名
#                 full_display_name = f"{seat_no}-{name}" 
...
#     # 修改：建立老師 (同時建立班級並產生代碼)
#     def create_teacher_with_class(self, account: str, password: str, name: str, school_id: int):
#         try:
#             with self.conn.cursor() as cursor:
#                 # 檢查 Email
...
#             # 4. 建立 Class (名稱先暫定為老師名字的班級，之後可改)
#             class_name_default = f"{name}的班級"
#             with self.conn.cursor() as cursor:
#                 cursor.execute("""
#                     INSERT INTO Class (school_id, teacher_id, class_name, class_code)
#                     VALUES (%s, %s, %s, %s);
#                 """, (school_id, teacher_id, class_name_default, class_code))

#             self.conn.commit()
#             return {"status": True, "class_code": class_code}
...
# # 修改：排行榜查詢 (改用當前登入學生的 class_id 查)
# @app.route('/leaderboard_class_query', methods=['GET'])
# def leaderboard_class_query():
#     db = SQL()
#     account = session.get('account')
#     # 先查出這個學生的 class_id
#     school_id, class_id = db.get_student_school_and_class_by_account(account)
...
#     # target_idx 在前端是用來標示自己的，這裡回傳 account (email)
#     return json.dumps({
#         "status": "success",
#         "data": result,
#         "target_idx": account 
...
#         # 接收新欄位
#         invite_code = str(obj.get('code', '')).strip().upper() # 代碼
#         account     = str(obj.get('account', '')).strip()      # Email
#         pwd         = str(obj.get('pwd', '')).strip()
#         name        = str(obj.get('name', '')).strip()
        
#         # 班級資訊 (例如 "3年2班")，僅作記錄用，實際綁定靠代碼
#         grade       = str(obj.get('grade', '')).strip()
#         clazz       = str(obj.get('clazz', '')).strip()
#         seat_no     = str(obj.get('seat_no', '')).strip()
        
#         if not invite_code:
#             return jsonify({"status": "error", "message": "請輸入班級代碼"}), 400
        
#         # 驗證代碼
#         class_id, school_id = db.validate_invite_code(invite_code)
#         if not class_id:
#             return jsonify({"status": "error", "message": "無效的班級代碼"}), 400
...
#             school_id=school_id,
#             seat_no=seat_no,
#             grade_class_str=grade_class_str
#         )
        
#         if not ok:
#             return jsonify({"status": "error", "message": "Email 已被註冊或系統錯誤"}), 400
...
#     except Exception as e:
#         print("stu_register_api error:", e)
#         return jsonify({"status": "error", "message": "系統發生錯誤"}), 500
...
#         # 建立老師並取得代碼
#         result = db.create_teacher_with_class(account=account, password=pwd, name=name, school_id=sid)
...
#                 "message": f"註冊成功！您的班級代碼是：{class_code}，請提供給學生註冊使用。"
#             })
#         else:
...
from routes.teacher import bp_teacher
from routes.student import bp_student  # [新增]

app = Flask(__name__)

app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(days=1)
app.secret_key = os.environ.get('SECRET_KEY', 'my_fixed_secret_key_123456')

# 註冊 Blueprints
app.register_blueprint(bp_auth)
app.register_blueprint(bp_teacher)
app.register_blueprint(bp_student) # [新增]
...
def before_request():
    ...
    # 如果是 Blueprint 的路由，通常 request.path 會包含 blueprint url_prefix (如果有的話)
    # 這裡我們簡單檢查路徑前綴
    path = request.path
